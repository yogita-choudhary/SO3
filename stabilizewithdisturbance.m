...
for n=1:N
    ...
    eo(:,n+1)=eo(:,n)+ew(:,n)*dt;
    %mo(:,n)=(1/(2*sqrt(1+trace(Rd(:,:,n)'*R(:,:,n)))))*(Vee(Rd(:,:,n)'*R(:,:,n)-R(:,:,n)'*Rd(:,:,n)));
    %mw(:,1)=E(:,:,n)*(omega(:,1)-R(:,:,1)'*Rd(:,:,1)*omegad(:,1));
    ...
    if n*dt>tf-0.005
      ...
      %udisc=-k1*abs(sqrt(sqrt(s(1,n)^2+s(2,n)^2+s(3,n)^2)))*sign(s(:,n))+v(:,n);
      v(:,n+1)=(-k3*(s(:,n)/sqrt(s(1,n)^2+s(2,n)^2+s(3,n)^2))-k4*s(:,n))*dt+v(:,n);
      %v(:,n+1)=v(:,n)+(-k2*sign(s(:,n)))*dt;
      unom=[0;0;0];
    else
        ...
        udisc=(-k1*s(:,n)/sqrt(sqrt(s(1,n)^2+s(2,n)^2+s(3,n)^2)))+v(:,n)-k2*s(:,n);
        %udisc=-k1*abs(sqrt(sqrt(s(1,n)^2+s(2,n)^2+s(3,n)^2)))*sign(s(:,n))+v(:,n);
        %v(:,n+1)=v(:,n)+(-k2*sign(ew(:,n)-ew(:,1)-integral))*dt;
        v(:,n+1)=(-k3*(s(:,n)/sqrt(s(1,n)^2+s(2,n)^2+s(3,n)^2))-k4*s(:,n))*dt+v(:,n);
    end
    g=unom+udisc;
...
hold on
plot([1 1]*5, ylim, '--k') 
legend('{e_{a_1}}','{e_{a_2}}','{e_{a_3}}')
xlabel('Time (s)','interpreter','latex') 
ylabel('Attitude error','interpreter','latex') 

figure(2)
plot(gem,euw(1,1:N+1),'DisplayName','e_{v_1}')
hold on 
plot(gem,euw(2,1:N+1),'DisplayName','e_{v_2}')
hold on 
plot(gem,euw(3,1:N+1),'DisplayName','e_{v_3}')
hold on
plot([1 1]*5, ylim, '--k') 
legend('{e_{v_1}}','{e_{v_2}}','{e_{v_3}}')
xlabel('Time (s)','interpreter','latex') 
...
figure(4)
plot(gem,Tc(1,1:N+1),'DisplayName','u_1')
hold on 
plot(gem,Tc(2,1:N+1),'DisplayName','u_2')
hold on 
plot(gem,Tc(3,1:N+1),'DisplayName','u_3')
hold on
plot([1 1]*5, ylim, '--k') 
legend('{u_1}','{u_2}','{u_3}')
xlabel('Time (s)','interpreter','latex') 
